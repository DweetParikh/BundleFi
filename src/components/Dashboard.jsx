...
export default function Dashboard() {
  ...
  const { getPrice, getChange24h, getChange7d, lastUpdated, loading } = usePrices()
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-3)' }}>
            {loading
              ...
              : <><Activity size={11} color="var(--green)" /> Prices updated {lastUpdated?.toLocaleTimeString()}</>
            }
          </div>
        </div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', padding: '16px 20px', marginBottom: 28, overflow: 'hidden', animation: 'fadeUp .4s .05s ease both' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          {!loading && <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--green)', animation: 'pulse 2s ease-in-out infinite' }} />}
        </div>
        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 }}>
          {TOP_PERFORMERS.map(t => {
            ...
            return (
              <div key={t.symbol} style={{ flexShrink: 0, padding: '10px 14px', borderRadius: 8, background: 'var(--bg-card2)', border: '1px solid var(--border)', textAlign: 'center', minWidth: 100 }}>
                ...
                {/* 24h change */}
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: ch == null ? 'var(--text-3)' : ch >= 0 ? 'var(--green)' : 'var(--red)', marginTop: 1 }}>
                  {ch != null ? `${ch >= 0 ? '+' : ''}${ch.toFixed(2)}%` : `+${t.change6m}% 6M`}
                  {ch != null && <span style={{ opacity: 0.6 }}> 24H</span>}
                </div>
                {/* 7d change */}
                {ch7 != null && (
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: ch7 >= 0 ? 'var(--green)' : 'var(--red)', marginTop: 1, opacity: 0.8 }}>
                    {ch7 >= 0 ? '+' : ''}{ch7.toFixed(2)}%<span style={{ opacity: 0.6 }}> 7D</span>
                  </div>
                )}
              </div>
            )
...
function BundleCard({ bundle, index, onInvest }) {
  ...
  // Weighted 24h bundle return (live)
  const avgChange24h = (() => {
  ...
  // Weighted 7d bundle return (live from CoinGecko)
  const avgChange7d = (() => {
  ...
  // Weighted 6M return from per-token static change6m values
  const avgChange6m = (() => {
    ...
    >
      ...
      <div style={{ padding: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
          ...
          {/* Return column — 6M + live 7D */}
          <div style={{ textAlign: 'right', fontFamily: 'var(--font-mono)', flexShrink: 0 }}>
            <div style={{ fontSize: 20, fontWeight: 600, color: avgChange6m != null ? 'var(--green)' : 'var(--text-3)' }}>
              {avgChange6m != null ? `+${avgChange6m.toFixed(1)}%` : 'N/A'}
            </div>
            <div style={{ fontSize: 10, color: 'var(--text-3)' }}>6M RETURN</div>
            {avgChange7d != null && (
              <div style={{ fontSize: 11, color: avgChange7d >= 0 ? 'var(--green)' : 'var(--red)', marginTop: 2 }}>
                {avgChange7d >= 0 ? '▲' : '▼'} {Math.abs(avgChange7d).toFixed(2)}% <span style={{ opacity: 0.7 }}>7D</span>
              </div>
            )}
            {avgChange24h != null && (
              <div style={{ fontSize: 10, color: avgChange24h >= 0 ? 'var(--green)' : 'var(--red)', marginTop: 1, opacity: 0.8 }}>
                {avgChange24h >= 0 ? '▲' : '▼'} {Math.abs(avgChange24h).toFixed(2)}% <span style={{ opacity: 0.7 }}>24H</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 5, marginBottom: 16, flexWrap: 'wrap' }}>
          {liveTokens.slice(0, 6).map(t => (
            <div key={t.symbol} style={{
              ...
              {t.icon} {t.symbol} <span style={{ color: 'var(--text-3)', fontSize: 9 }}>{t.weight}%</span>
              <span style={{ color: 'var(--text-1)', marginLeft: 4 }}>{formatPrice(t.livePrice)}</span>
              {t.change24h != null && (
                <span style={{ color: t.change24h >= 0 ? 'var(--green)' : 'var(--red)', marginLeft: 3 }}>
        ...
        {/* Stats row: AUM / 7D (live) / INVESTORS */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 16, padding: 12, borderRadius: 'var(--r)', background: 'var(--bg-card2)', border: '1px solid var(--border)' }}>
          {[
            ...
            {
              label: '7D LIVE',
              value: avgChange7d != null ? formatPercent(avgChange7d) : (bundle.change7d != null ? formatPercent(bundle.change7d) : '—'),
              green: (avgChange7d ?? bundle.change7d ?? 0) > 0,
              live:  avgChange7d != null,
            },
            ...
            <div key={s.label} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', marginTop: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
                {s.live && <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--green)' }} />}
                {s.label}
              </div>
            </div>
          ))}
