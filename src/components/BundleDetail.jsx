...
export default function BundleDetail() {
  ...
  const { getPrice, getChange24h, getChange7d, lastUpdated } = usePrices()
  ...
  // Enrich tokens with live prices + changes
  ...
  // Weighted bundle 24h return (live)
  ...
  // Weighted bundle 7d return (live from CoinGecko)
  ...
  // Weighted 6M from per-token static change6m values
  const weightedChange6m = (() => {
    const known = liveTokens.filter(t => t.change6m != null)
    if (!known.length) return bundle.apy ?? null
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ animation: 'fadeUp .4s ease both' }}>
          ...
          {lastUpdated && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-3)' }}>
              ...
              Prices live · Updated {lastUpdated.toLocaleTimeString()} · Jupiter + CoinGecko
            </div>
          )}
        </div>
      ...
      {/* Stats cards — 6M, 24H (live), 7D (live), AUM, INVESTORS, RISK */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(155px,1fr))', gap: 12, marginBottom: 32, animation: 'fadeUp .4s .05s ease both' }}>
        {[
          {
            label: '6M RETURN',
            value: weightedChange6m != null ? formatPercent(weightedChange6m) : '—',
            green: (weightedChange6m ?? 0) > 0,
            live:  false,
          },
          {
            label: '24H BUNDLE',
            value: weightedChange24h != null ? formatPercent(weightedChange24h) : '—',
            green: (weightedChange24h ?? 0) > 0,
            live:  weightedChange24h != null,
          },
          {
            label: '7D BUNDLE',
            value: weightedChange7d != null ? formatPercent(weightedChange7d) : (bundle.change7d != null ? formatPercent(bundle.change7d) : '—'),
            green: (weightedChange7d ?? bundle.change7d ?? 0) > 0,
            live:  weightedChange7d != null,
          },
          { label: 'TOTAL AUM',  value: formatCurrency(bundle.aum), live: false },
          { label: 'INVESTORS',  value: bundle.investors, live: false },
          { label: 'RISK',       value: bundle.risk || '—', riskColor: RISK_COLORS[bundle.risk], live: false },
        ].map(s => (
          <div key={s.label} style={{ padding: '18px 20px', borderRadius: 'var(--r-lg)', background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
              <div style={{ fontSize: 10, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '0.07em' }}>{s.label}</div>
              {s.live && <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--green)', flexShrink: 0 }} />}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {/* Token breakdown — live prices + 24h + 7d */}
          <div style={{ padding: 24, borderRadius: 'var(--r-lg)', background: 'var(--bg-card)', border: '1px solid var(--border)', animation: 'fadeUp .4s .15s ease both' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              {lastUpdated && <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--green)', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--green)', display: 'inline-block' }} /> LIVE</span>}
            </div>
                ...
                <div key={t.symbol} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 'var(--r)', background: 'var(--bg-card2)', border: '1px solid var(--border)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 8 }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, fontFamily: 'var(--font-mono)', fontSize: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                        {/* Live price — largest */}
                        <span style={{ color: 'var(--text-1)', fontWeight: 700, fontSize: 14 }}>{formatPrice(t.livePrice)}</span>
                        {/* 24h change */}
                        {t.change24h != null && (
                          <span style={{
                            padding: '2px 7px', borderRadius: 4, fontSize: 11,
                            background: t.change24h >= 0 ? 'var(--green-dim)' : 'rgba(255,51,102,0.1)',
                        ...
                        {t.change7d != null && (
                          <span style={{
                            padding: '2px 7px', borderRadius: 4, fontSize: 11,
                            background: t.change7d >= 0 ? 'var(--green-dim)' : 'rgba(255,51,102,0.1)',
                            color: t.change7d >= 0 ? 'var(--green)' : 'var(--red)',
                          }}>
          ...
          <div style={{ padding: 24, borderRadius: 'var(--r-lg)', background: 'var(--bg-card)', border: '1px solid var(--border)', animation: 'fadeUp .4s .1s ease both' }}>
            ...
            <PieChart width={280} height={200}>
              ...
              <Tooltip content={({ active, payload }) => {
                ...
                const ch7 = getChange7d(t.symbol)
                return (
                  <div style={{ background: 'var(--bg-card2)', border: '1px solid var(--border-md)', borderRadius: 6, padding: '8px 12px', fontFamily: 'var(--font-mono)', fontSize: 11 }}>
                    <div style={{ color: t.color }}>{t.icon} {t.symbol} — {t.weight}%</div>
                    <div style={{ color: 'var(--text-1)', marginTop: 4, fontWeight: 700 }}>{formatPrice(lp)}</div>
                    {ch != null && (
                      <div style={{ color: ch >= 0 ? 'var(--green)' : 'var(--red)', marginTop: 2 }}>
                        {ch >= 0 ? '+' : ''}{ch.toFixed(2)}% 24H
                      </div>
                    )}
                    {ch7 != null && (
                      <div style={{ color: ch7 >= 0 ? 'var(--green)' : 'var(--red)', marginTop: 2 }}>
                        {ch7 >= 0 ? '+' : ''}{ch7.toFixed(2)}% 7D
                      </div>
                    )}
                  </div>
                )
