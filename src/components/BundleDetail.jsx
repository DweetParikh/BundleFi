...
import { ArrowLeft, TrendingUp, Users, Calendar, Shield } from 'lucide-react'
import {
  ResponsiveContainer, AreaChart, Area, XAxis, YAxis, Tooltip, CartesianGrid,
  PieChart, Pie, Cell, Legend,
} from 'recharts'
import { useApp } from '../context/AppContext'
import { formatCurrency, formatPercent, RISK_COLORS } from '../data/mockData'
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'40px 24px' }}>
        ...
        <div style={{ animation:'fadeUp .4s ease both' }}>
          <div style={{ display:'flex', gap:8, marginBottom:10, flexWrap:'wrap' }}>
            <span style={{
              ...
              background:`${bundle.color || 'var(--cyan)'}18`,
              color: bundle.color || 'var(--cyan)',
              border:`1px solid ${bundle.color || 'var(--cyan)'}30`,
            ...
            {isInvested && (
              ...
              }}>● YOU'RE INVESTED</span>
            )}
          </div>
          <h1 style={{ fontFamily:'var(--font-display)', fontWeight:800, fontSize:'clamp(26px,4vw,40px)', color:'var(--text-1)', marginBottom:8 }}>
            {bundle.name}
          </h1>
          <p style={{ color:'var(--text-2)', fontSize:14, maxWidth:560, lineHeight:1.7 }}>
            {bundle.description}
          </p>
        </div>
      ...
      <div style={{
        ...
        gap:14, marginBottom:36, animation:'fadeUp .4s .05s ease both',
      }}>
        {[
          { label:'6M Return',   value: bundle.apy    != null ? formatPercent(bundle.apy) : '—', green: bundle.apy > 0, icon: TrendingUp },
          { label:'7D Return',   value: bundle.change7d  != null ? formatPercent(bundle.change7d) : '—',  green: bundle.change7d > 0,  icon: TrendingUp },
          { label:'30D Return',  value: bundle.change30d != null ? formatPercent(bundle.change30d) : '—', green: bundle.change30d > 0, icon: TrendingUp },
          { label:'Total AUM',   value: formatCurrency(bundle.aum), icon: TrendingUp },
          { label:'Investors',   value: bundle.investors, icon: Users },
          { label:'Risk Level',  value: bundle.risk || '—', riskColor: RISK_COLORS[bundle.risk], icon: Shield },
        ].map(s => (
          <div key={s.label} style={{
            padding:'18px 20px', borderRadius:'var(--r-lg)',
            background:'var(--bg-card)', border:'1px solid var(--border)',
          }}>
            <div style={{ fontSize:11, color:'var(--text-3)', fontFamily:'var(--font-mono)', marginBottom:6, letterSpacing:'0.07em' }}>
              {s.label.toUpperCase()}
            </div>
            <div style={{
              fontFamily:'var(--font-mono)', fontWeight:600, fontSize:20,
              color: s.riskColor || (s.green ? 'var(--green)' : 'var(--text-1)'),
            }}>{s.value}</div>
          </div>
        ))}
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:24, alignItems:'start', flexWrap:'wrap' }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={bundle.chartData}>
                  ...
                  <Area
                    ...
                    stroke={bundle.color || '#00d4ff'} strokeWidth={2}
                    fill="url(#detailGrad)" dot={false}
                    activeDot={{ r:4, fill: bundle.color || '#00d4ff', strokeWidth:0 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                      ...
                      <div style={{ display:'flex', gap:16, fontFamily:'var(--font-mono)', fontSize:12 }}>
                        <span style={{ color:'var(--text-2)' }}>${t.price < 0.001 ? t.price.toFixed(6) : t.price < 1 ? t.price.toFixed(4) : t.price.toFixed(2)}</span>
                        {t.change6m != null && (
                    ...
                    <div style={{ height:3, background:'var(--border)', borderRadius:2, overflow:'hidden' }}>
                      <div style={{
                        ...
                        width:`${t.weight}%`,
                        background:`linear-gradient(90deg, ${t.color || '#00d4ff'}, ${t.color || '#00d4ff'}88)`,
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize:12, fontFamily:'var(--font-mono)', color:'var(--text-3)', marginBottom:12, letterSpacing:'0.07em' }}>
              ALLOCATION
            </div>
            <PieChart width={280} height={200}>
              ...
              >
                {bundle.tokens.map(t => (
                  <Cell key={t.symbol} fill={t.color || '#00d4ff'} stroke="var(--bg-card)" strokeWidth={2} />
                ))}
              </Pie>
              <Tooltip
                content={({ active, payload }) => {
          ...
          {isInvested && (
            <div style={{
              ...
              <div style={{ fontSize:11, fontFamily:'var(--font-mono)', color:'var(--green)', marginBottom:14, letterSpacing:'0.07em' }}>
                YOUR POSITION
              </div>
              {[
                { label:'Invested',      value:`$${position.invested.toFixed(2)}` },
                { label:'Current Value', value:`$${position.currentValue.toFixed(2)}` },
                { label:'P&L',           value:`+$${(position.currentValue - position.invested).toFixed(2)}`, green:true },
                ...
                <div key={r.label} style={{
                  ...
                  <span style={{
                    ...
                    color: r.green ? 'var(--green)' : 'var(--text-1)',
                  }}>{r.value}</span>
                </div>
              ))}
            </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize:11, fontFamily:'var(--font-mono)', color:'var(--text-3)', marginBottom:12, letterSpacing:'0.07em' }}>
              BUNDLE INFO
            </div>
            {[
